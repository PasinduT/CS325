...
def output_tree (the_tree, name, filename):
    ...
    style = TreeStyle()
    style.title.add_face(TextFace(name, fsize=3), column=0)
    ...
    ancenstor = t.get_common_ancestor("'Sand lizard'", "'Common ostrich'")
    t.set_outgroup(ancenstor)
    ...
    # automatically.
    t.render(filename, tree_style=style, w=720, units="mm")
...
def main() :
    file = open('output/tree_bs_RHO.txt')
    data = file.read()
    file.close()

    output_tree(data, 'RHO rooted', 'mytree.png')
